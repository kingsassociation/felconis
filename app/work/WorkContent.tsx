  ...
  return (
    <div className="min-h-screen bg-white text-text-primary">
      ...
      <main className="grain pt-4 md:pt-8">
        {/* INSTITUTIONAL HERO */}
        <section className="relative py-4 md:py-8 md:pb-20 border-b border-stroke overflow-hidden text-center">
          <div className="absolute inset-x-0 top-0 h-[600px] bg-gradient-to-b from-brand/[0.03] to-transparent pointer-events-none -z-10" />

          <div className="container-max relative z-10 space-y-8 md:space-y-12 text-center">
            <motion.div
            ...
            >
              ...
              <h1 className="text-5xl md:text-8xl lg:text-9xl font-black tracking-tighter uppercase leading-[0.9] text-text-primary">
                STRATEGIC <br /> <span className="text-brand">RESULTS.</span>
              </h1>
              <p className="text-text-secondary text-base md:text-xl font-medium leading-relaxed max-w-2xl mx-auto">
              ...
              <div className="flex flex-wrap gap-2">
                {CATEGORIES.map(cat => (
                  <button
                    ...
                    className={`px-5 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${activeCategory === cat
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12">
              {filteredStudies.map((study, i) => (
                <motion.div
                  ...
                  className="group"
                >
                  <Link href={`/work/${study.slug}`} className="block space-y-6 md:space-y-8">
                    <div className="aspect-[4/5] rounded-[2rem] overflow-hidden border border-stroke bg-surface relative group-hover:border-brand/40 transition-all duration-700">
                      <img src={getCloudinaryUrl(study.image)} alt={study.title} className="w-full h-full object-cover transition-all duration-1000 group-hover:scale-105" />
